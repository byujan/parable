...
export async function POST(
  ...
  // Stop the agent executor if running
  if (campaign.agent_session_id) {
    const executor = getExecutor(campaign.agent_session_id)
    if (executor) {
      executor.stop()
    }
    await updateSessionStatus(campaign.agent_session_id, 'stopped')
  }
  ...
  await supabase
    ...
    .update({
      ...
      cancel_reason: 'Manually stopped by admin',
    })
    .eq('id', params.id)

  return NextResponse.json({ status: 'cancelled', campaign_id: params.id })
}
