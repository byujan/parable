...
export async function POST(
  ...
  // Stop the executor (it will be recreated on resume)
  if (campaign.agent_session_id) {
    const executor = getExecutor(campaign.agent_session_id)
    if (executor) {
      executor.stop()
    }
    await updateSessionStatus(campaign.agent_session_id, 'pending')
  }
  ...
  return NextResponse.json({ status: 'paused', campaign_id: params.id })
}
